-- 見積書印刷用の列追加と save_estimate() の差し替え（docs/print-design.md）
-- 冪等: 何度流しても同じ状態になる
...
alter table public.estimates
  add column if not exists estimate_no   text,   -- 見積番号 YYYYMMDD-NN（新規保存時に自動採番）
  add column if not exists valid_until   date,   -- 有効期限
  add column if not exists customer_note text;   -- 顧客向け備考（判断理由メモとは別）

create unique index if not exists estimates_estimate_no_key
  on public.estimates (estimate_no) where estimate_no is not null;

alter table public.estimate_materials
  add column if not exists unit text not null default '個';  -- 数量の単位

-- ---------------------------------------------------------------
-- 2. 見積番号の採番: 同じ見積日の中で 01 から連番
-- ---------------------------------------------------------------
create or replace function public.next_estimate_no(p_date date)
returns text
language plpgsql
security invoker
...
declare
  ...
  v_seq    integer;
begin
  -- 同時保存の競合を防ぐため、日付ごとに advisory lock を取る
  perform pg_advisory_xact_lock(hashtext('estimate_no:' || v_prefix));

  select coalesce(max(substring(estimate_no from length(v_prefix) + 1)::integer), 0) + 1
    into v_seq
    from public.estimates
   where estimate_no like v_prefix || '%'
     and substring(estimate_no from length(v_prefix) + 1) ~ '^\d+$';

  return v_prefix || lpad(v_seq::text, 2, '0');
end;
$$;

revoke all on function public.next_estimate_no(date) from public, anon;
grant execute on function public.next_estimate_no(date) to authenticated;

-- ---------------------------------------------------------------
-- 3. save_estimate() 差し替え（新列・単位・採番対応）
-- ---------------------------------------------------------------
...
begin
  ...
  if p_id is null then
    insert into public.estimates (
      ...
      risk_alerts, status,
      estimate_no, valid_until, customer_note
    ) values (
